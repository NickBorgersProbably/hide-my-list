...
If .reminder-signal exists but the main session has no attached user-facing surface, reply with ONLY: NO_REPLY and leave .reminder-signal in place for the next eligible run.

If .reminder-signal exists and the main session has an attached user-facing surface:
1. Read the file and deliver each reminder to the user on the existing `main` session surface.
