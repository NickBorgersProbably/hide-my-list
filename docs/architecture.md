...
**Timezone handling:** `setup/openclaw.json.template` pins `agents.defaults.envelopeTimezone` to `America/Chicago`, so OpenClaw injects the prompt envelope in the user's local timezone. AI converts user times (e.g., "6pm PT", "3pm Central") to full ISO 8601 with timezone offsets at intake. Both the one-shot cron's `schedule.at` field and the polling check script compare against UTC — no timezone conversion at fire/check time. `scripts/user-time-context.sh` remains available for cron or edge-case conversions when a stored UTC instant must be described in user-local calendar language.
