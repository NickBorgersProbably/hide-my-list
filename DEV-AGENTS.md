...
- `app/tools/reminders.py` — Reminder outbox CRUD and COMPLETE's Postgres access; `cancel_pending_for_page` marks a page's pending/scheduled `kind='reminder'` rows `dead` (`last_error='completed by user'`) when the user completes a reminder before it fires, and `cancel_pending_reminders` wraps it in its own connection and commit. `cancel_pending_nudges_for_page` marks a completed task's pending/scheduled `kind='deadline'` rows `dead` (`last_error='task completed'`) and supersedes the series' active `reminder_scheduling_ledger` rows; `cancel_pending_nudges` wraps it and is called, best-effort, by every completion write (`complete_node`, `_log_finished.py`, the interaction review's `complete_task`). `load_recent_outbound` returns the peer's newest delivery awaiting a reply (incl. `reminder_type`); `resolve_recent_outbound` clears every live delivery row for a completed page
...
- `app/graph/nodes/rejection.py` — REJECT intent node. With no active task and no titled ledger suggestion from the last 24 h it runs no prompt, touches neither Notion nor the ledger, and replies with the fixed `NOTHING_ACTIVE_BODY` (`rejection_node.nothing_active`). A `{task}` token is rendered only from a listed, titled alternative; otherwise the sentences carrying it are dropped (`NO_ALTERNATIVE_BODY` when nothing is left, `rejection_node.orphan_task_token`). Records the declined page as `rejected` and a listed, titled alternative as `suggested` in the recent-task ledger; prompt carries history + recent-task ledger
...
- `app/graph/nodes/_log_finished.py` — Shared "log an accomplishment" path used by intake (`already_finished` saves) and complete (a yes to the unlisted-report log question): completes the open task a title duplicates at the intake dedup threshold or creates the page Completed; when completing a dedup match, calls `reminders.cancel_pending_nudges` and `reminders.resolve_recent_outbound`, each in a best-effort try/except, to stop its deadline nudges and clear stale delivery rows (mirroring `complete_node`); calls `maybe_reward` with complete_node's kwargs, records a `completed` ledger event, bumps streak/`tasks_completed_today`, clears `pending_clarification`, and replies with `_celebration_body` + `notion_page_title`. Logs booleans/counts only under the caller's event name
...
- `app/scheduler/reminder_worker.py` — SELECT FOR UPDATE SKIP LOCKED worker; completes Notion only for `reminder_outbox.kind='reminder'`; writes `recent_outbound.reminder_type` from the row's `kind` so a deadline nudge is never mistaken for an already-completed reminder; before sending a row of either kind it reads the page and marks the row `dead` (`last_error='page already completed'`) instead of sending when the page is already Completed (fail-open: a failed read sends and logs `reminder_worker.presend_check_failed`)
