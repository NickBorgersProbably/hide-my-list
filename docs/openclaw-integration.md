...
- Reminder delivery still happens in agent context, but `scripts/check-reminders.sh` hands due reminders to the cron prompt through the repo-root reminder handoff file (default filename: `.reminder-signal`, overridable via `REMINDER_SIGNAL_FILE`) instead of relying on a long-running daemon
