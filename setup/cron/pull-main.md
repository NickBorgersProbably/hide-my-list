...
Isolated cheap-tier maintenance session (see `setup/model-tiers.json`). Executes `scripts/pull-main.sh`, stays silent (`NO_REPLY`). Cron spec re-application after pulls handled by heartbeat drift correction (`docs/heartbeat-checks.md` Check 2b), not this job — isolated session can't reliably call CronList/CronUpdate.
...
- The `heartbeat` cron (`docs/heartbeat-checks.md` Checks 2/2b) re-registers missing jobs + patches drift — guards against manual deletion, gateway data loss, or other failure modes that drop the canonical job.
- Post-pull cron spec re-application handled by heartbeat within next 2-hour cycle. Delay acceptable — cron spec changes rare, not user-facing.
