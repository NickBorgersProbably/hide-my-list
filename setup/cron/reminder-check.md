...
Both delivery paths do that delivery with the `message` tool (`action: send`, `channel: signal`, `target: channels.signal.defaultTo` from `openclaw.json`), then call `scripts/notion-cli.sh complete-reminder PAGE_ID sent|missed` only after the send succeeds so Notion `Status`, `Reminder Status`, and `Completed At` reflect confirmed delivery rather than attempted delivery. If the target is missing or invalid, delivery fails closed and the handoff file stays in place. `TOOLS.md` defines the canonical contract.
