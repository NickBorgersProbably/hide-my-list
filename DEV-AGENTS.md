...
- `app/tools/interaction_reviews.py` — Interaction review verdict store: `insert_review` (one row per review outcome) and `count_executed_corrections` (per-peer hourly rate limit, all-peer 24 h alert count). The review's only Postgres access
...
- `app/graph/interaction_review.py` — Post-send interaction review, run by the Signal listener as a background task after each turn (not a graph node). `review_turn` reads the finished turn, calls the medium model (`judge_turn`, caller `interaction_review`), validates the strict JSON verdict (`parse_verdict`: action enum, page ids from the shown lists only, `reopen_task` only for this turn's completions, `{task}` in every follow-up, no blame phrasing), runs at most one action, sends one follow-up through `render_task_token`, and writes the checkpoint as the `send` node. Rate limit + ops alert + verdict store via `app/tools/interaction_reviews.py`
...
- `migrations/0014_interaction_reviews.sql` — Adds `interaction_reviews` (one row per post-send review outcome: verdict `ok|correct|skipped|error`, action, target page, executed, follow-up sent) with an index on `(peer, created_at)`; `reason` is private text
