...
- `app/graph/nodes/intake.py` — ADD_TASK intent node; confirmation is `{task}` + stated deadline/reminder time + optional first step (sub-tasks are stored, never listed). A model `action: "already_done"` saves nothing and returns `complete_node(state)`
