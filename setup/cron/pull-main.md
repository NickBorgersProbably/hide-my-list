...
Isolated cron maintenance session. Executes `scripts/pull-main.sh`, stays silent (`NO_REPLY`). Cron spec re-application after pulls handled by heartbeat drift correction (HEARTBEAT.md Check 2b), not this job — isolated session can't reliably call CronList/CronUpdate.
