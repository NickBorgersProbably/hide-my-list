...
- If `HANDOFF_FILE` still exists when heartbeat runs, atomically claim it first by renaming it to a unique repo-root sibling such as `.reminder-signal-$TIMESTAMP-$PID.claimed`; only the session that wins the rename may deliver those reminders
- After a successful claim, read the claimed file, send each reminder to the user, run `scripts/notion-cli.sh complete-reminder PAGE_ID sent|missed` based on the file, then delete the claimed file
- If delivery fails after claim, restore the claimed file back to `HANDOFF_FILE` if that path is absent; if a fresh handoff file already exists, keep the claimed file for operator attention rather than double-delivering
