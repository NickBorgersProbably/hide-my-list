...
| `OPS_ALERT_SIGNAL_NUMBER` | No | Operator's Signal number for heartbeat ops alerts (malformed reminder handoff, Notion down, cron expiry, cron spec drift, env missing, dirty-pull failures). If unset, ops alerts are silently skipped. |
...
- Heartbeat ops alerts (malformed reminder handoff, Notion down, cron expiry, cron spec drift, env missing, dirty-pull failures) are sent explicitly to the operator's Signal number — set `OPS_ALERT_SIGNAL_NUMBER` in `.env` to enable them. This is separate from Caroline's thread.
