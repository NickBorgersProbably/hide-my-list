...
Registering this one-shot cron at intake satisfies the supported contract. Reminder confirmations still avoid the guard regex (`I'll remind you ...`, `I'll set a reminder`, etc.) as defense in depth, so a framework tracking miss cannot leak the note into user-facing copy. This one-shot cron also delivers reminders at exact wall-clock time instead of relying on the up-to-~75-min worst-case latency of the polling backstop.
