...
- `app/tools/reminders.py` — Reminder outbox CRUD; `cancel_pending_for_page` marks a page's pending/scheduled `kind='reminder'` rows `dead` (`last_error='completed by user'`) when the user completes a reminder before it fires
...
- `app/scheduler/reminder_worker.py` — SELECT FOR UPDATE SKIP LOCKED worker; completes Notion only for `reminder_outbox.kind='reminder'`; writes `recent_outbound.reminder_type` from the row's `kind` so a deadline nudge is never mistaken for an already-completed reminder
