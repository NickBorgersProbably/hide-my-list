...
Use `set` when the in-turn `CronCreate` call succeeded and `saved` when it failed. Both phrasings intentionally avoid "I'll remind you ..." and similar first-person reminder commitments, because OpenClaw's reminder-guard is a post-process on reply text and false positives should not leak into the visible confirmation. The reminder is still saved in Notion either way; if the primary scheduler call failed, the safety net catches it at the next 15-min poll.
