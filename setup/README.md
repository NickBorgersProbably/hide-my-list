...
Cron jobs auto-expire after 7 days. The heartbeat re-registers them automatically. Each cron injects a `systemEvent` into the main agent session with `delivery: { mode: none }`, so the main agent decides whether there is anything worth surfacing.
