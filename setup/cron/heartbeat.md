...
Runs daily via OpenClaw's durable cron system. This replaces the built-in OpenClaw heartbeat configured under `agents.defaults.heartbeat`; the template keeps that built-in path disabled with `every: 0`.

## Registration

```
CronCreate:
  schedule: "0 9 * * *"  # daily at 04:00 CT — caroline-asleep window
  durable: true
  name: "heartbeat"
...
- `heartbeat` belongs to the canonical recurring cron catalog with `reminder-check`, `pull-main`, and `janitor`. Daily Check 2 in `docs/heartbeat-checks.md` verifies all four jobs exist. Weekly janitor Check 2b compares full live specs and patches drift.
