...
- `app/graph/context.py` — Recent-task ledger and shared prompt context. intake, selection, complete, and rejection write the ledger only through `record_task_event` (dedupe by page, newest wins, known title kept, 7-day prune, cap 8); `render_history` (8 messages × 400 chars) and `render_recent_tasks` render prompt blocks; `hydrate_context` is the graph entry node that merges the peer's recent `recent_outbound` deliveries as `reminded`/`nudged` entries. Fail-soft: a DB error keeps the existing ledger and never reaches the classifier's error fallback
...
- `app/graph/nodes/rejection.py` — REJECT intent node; a named alternative it offers becomes the active task (marked In Progress), the same as a selection suggestion
