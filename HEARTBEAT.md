...
- This is the primary guaranteed delivery path for reminders. The isolated `reminder-check` cron only writes `.reminder-signal` — it does not deliver. Delivery happens here (every 60 min) and opportunistically via the AGENTS.md startup check (on every user interaction).
...
| Job | Schedule | Action |
|-----|----------|--------|
| reminder-check | `*/15 * * * *` | Run `scripts/check-reminders.sh` (query-only; writes `.reminder-signal` if reminders due) |
