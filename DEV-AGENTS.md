...
- `docs/ai-prompts/interaction-review.md` — Post-send interaction review: inputs, strict JSON verdict schema, correction policy (`complete_task` or `send_only` only; never creates or reopens), the fixed follow-up templates, job states and startup rules
...
- `app/tools/reminders.py` — Reminder outbox CRUD and COMPLETE's Postgres access; `cancel_pending_for_page` marks a page's pending/scheduled `kind='reminder'` rows `dead` (`last_error='completed by user'`) when the user completes a reminder before it fires, and `cancel_pending_reminders` wraps it in its own connection and commit. `load_recent_outbound` returns the peer's newest delivery awaiting a reply (incl. `reminder_type`); `resolve_recent_outbound` clears every live delivery row for a completed page
...
- `app/tools/interaction_reviews.py` — Interaction review job store: `create_pending` (durable row before a review runs), `claim` (`pending` → `executing` with action + page, before the first external effect), `mark_executed` / `mark_follow_up_sent` (each effect stored as it lands), `finalize` (idempotent move from `pending|executing` to `ok|correct|skipped|error`; keeps what a claim or mark stored; no-op on a final row), `list_unfinished` (startup), and `count_executed_corrections` (per-peer hourly rate limit, all-peer 24 h alert count). The review's only Postgres access
...
- `app/graph/context.py` — Recent-task ledger and shared prompt context. intake, selection, rejection, and complete write the ledger only through `record_task_event` (dedupe by page, newest wins, known title kept, 7-day prune, cap 8); `render_history` (8 messages × 400 chars) and `render_recent_tasks` render prompt blocks; `record_turn_action` appends to `turn_actions` (what this turn's node did: `notion.create_task`, `notion.create_reminder`, `notion.update_status`, `notion.update_property`, `reminder.cancel`, `suggest`, `reward`, `clarify`; written by intake, selection, complete, rejection, and `_log_finished.py`); `hydrate_context` is the graph entry node that resets `turn_actions` and merges the peer's recent `recent_outbound` deliveries as `reminded`/`nudged` entries and reads the stored title of an untitled delivery page from Notion (at most 3 per turn, 5 s each). Fail-soft: a DB error keeps the existing ledger, a failed title read leaves that entry untitled, and neither reaches the classifier's error fallback
- `app/graph/routing.py` — Intent classification + conditional edges; owns the `pending_clarification` lifecycle. `classify_intent` runs after `hydrate_context` on every turn. The classify prompt carries the prior-conversation window, the `Recent tasks:` ledger block, and a conversation-state / awaiting-clarification line. A live clarification (`complete_target` or `unlisted_report`) steers a CHAT- or COMPLETE-classified message to `complete_node` as the answer; any other intent, an expired timestamp, or malformed state drops it. A whole-message positional/affirmative reply routes to COMPLETE without a model call; a bare negative never does — it clears the clarification with "Got it, leaving that open.", except at stage 1 of an `unlisted_report` with a proposed title, where it offers "Got it. Want me to log '<title>' as done?" and stores stage 2 (no candidates, attempts 2)
- `app/graph/interaction_review.py` — Post-send interaction review, run by the Signal listener as a background task after each turn (not a graph node). `review_turn` reads the finished turn, calls the medium model (`judge_turn`, caller `interaction_review`), validates the strict JSON verdict (`parse_verdict`: keys `verdict|reason|action|page_id`, action `none|complete_task|send_only`, page ids from the shown lists only), claims the row before the first external effect, runs at most one action (`complete_task` reads the page's work type, energy, and time estimate for `maybe_reward`), and — only while the thread's latest checkpoint id still equals `turn_ref` (else `error(stale_checkpoint)`) — sends the action's fixed `FOLLOW_UP_TEMPLATES` text through `render_task_token` and writes the checkpoint as the `send` node. The model writes no user-facing text. Every exit path finalizes the job row passed in as `review_id`, cancellation included (skip reasons `buffer_non_empty|superseded|cancelled|rate_limited|disabled|timeout`). Rate limit + ops alert + job store via `app/tools/interaction_reviews.py`
- `app/graph/nodes/intake.py` — ADD_TASK intent node; confirmation is `{task}` + stated deadline/reminder time + optional first step (sub-tasks are stored, never listed). A model `action: "already_done"` saves nothing and returns `complete_node(state)`. A save marked `"already_finished": true` (the user logs something already done) creates the page Completed — or completes the open task it duplicates — runs `maybe_reward`, and replies with complete.py's `_celebration_body`, through the shared `_log_finished.py`
- `app/graph/nodes/selection.py` — GET_TASK intent node. The prompt carries the incoming message (never history) so time and mood come from it when state has none; with no duration, short tasks score higher and none are excluded on time. An id outside the candidate list or a blank-titled page is no selection: no write, no active task, a neutral retry line, shape-only log
- `app/graph/nodes/chat.py` — CHAT intent node
- `app/graph/nodes/rejection.py` — REJECT intent node; records the declined page as `rejected` and a listed, titled alternative as `suggested` in the recent-task ledger; prompt carries history + recent-task ledger
- `app/graph/nodes/cannot_finish.py` — CANNOT_FINISH intent node; prompt carries history + recent-task ledger
- `app/graph/nodes/need_help.py` — NEED_HELP intent node; prompt carries history + recent-task ledger. With no active task, helps with the newest titled `added`/`suggested` ledger entry
- `app/graph/nodes/check_in.py` — CHECK_IN intent node
- `app/graph/nodes/complete.py` — COMPLETE intent node. Resolution order: a task named in the message (open tasks and not-yet-fired reminders; model confirms at ≥0.90), then the newest context pooled from the recent-task ledger, `recent_outbound`, and `active_task` (two tasks within 15 min ⇒ ask), then a clarification. When the match model finds no candidate and reports `names_unlisted_task` over a widened or empty list (a specific finished task on none of the candidates), the node asks with a yes/no choice and stores an `unlisted_report` clarification — "Nice one! Want me to log '<title>' as done?" when the model's `unlisted_task_title` shares a task-naming word with the message, or "Nice one! I've left your list as it is." otherwise — and never completes a context task on that report (`complete_node.unlisted_report`). Over a scored shortlist a null match keeps the existing `complete_target` question naming those overlapping options. Answering an `unlisted_report` clarification: "yes" logs the title through `_log_finished.py` (`complete_node.logged_finished`); context sources are never consulted, and an answer naming no task closes the question. Every completion writes Status to Completed in Notion; a delivered reminder's write is an idempotent repair (delivery already wrote it but may have failed). Completing a reminder also cancels its pending outbox rows (retried once; a second failure logs `complete_node.reminder_cancel_failed`, enqueues a `reminder_cancel_failed` ops alert, and still completes). Reaches Postgres only through `app/tools/reminders.py`. An answer to a clarification that types a title back (Dice ≥ 0.85, unique) resolves without a model call. The celebration body is `{task} — done. <reward>` with `notion_page_title` set when a title is known; when no title can be read, the reward text alone goes out
- `app/graph/nodes/send.py` — Terminal send node; enforces the task-naming invariant on every draft carrying `notion_page_title`
- `app/graph/nodes/_task_match.py` — Shared open-task extraction, token shortlist, and model-response parsing helper used by ADD_TASK (duplicate detection) and COMPLETE (title-match resolution). `open_tasks(response, include_reminders=...)` returns `{id, title, kind}`; COMPLETE includes open reminder pages, intake's `open_non_reminder_tasks` wrapper excludes them. The score ranks candidates for the model; it never decides on its own that the model sees nothing — COMPLETE re-runs it unfiltered over the whole open list when nothing clears the threshold
- `app/graph/nodes/_log_finished.py` — Shared "log an accomplishment" path used by intake (`already_finished` saves) and complete (a yes to the unlisted-report log question): completes the open task a title duplicates at the intake dedup threshold or creates the page Completed; when completing a dedup match, calls `reminders.resolve_recent_outbound` in a best-effort try/except to clear stale delivery rows (mirroring `complete_node`); calls `maybe_reward` with complete_node's kwargs, records a `completed` ledger event, bumps streak/`tasks_completed_today`, clears `pending_clarification`, and replies with `_celebration_body` + `notion_page_title`. Logs booleans/counts only under the caller's event name
...
- `app/scheduler/reminder_worker.py` — SELECT FOR UPDATE SKIP LOCKED worker; completes Notion only for `reminder_outbox.kind='reminder'`; writes `recent_outbound.reminder_type` from the row's `kind` so a deadline nudge is never mistaken for an already-completed reminder; before sending a `kind='reminder'` row it reads the page and marks the row `dead` (`last_error='page already completed'`) instead of sending when the page is already Completed (fail-open: a failed read sends and logs `reminder_worker.presend_check_failed`)
- `app/scheduler/deadline_planner.py` — Pure deadline milestone planner and quiet-hours/load-balancing slot assignment; `format_reminder_summary` names the earliest slot only (`First nudge <local time>.`)
- `app/scheduler/reminder_scheduling.py` — Shared deadline reminder scheduler helper; writes `reminder_scheduling_ledger`, deadline outbox rows (bodies name the task when available, or emit a generic body when the stored title is absent; `title` is threaded from intake and the daily backstop), and private page-to-peer routing metadata
...
- `app/ingress/signal_listener.py` — Authorized Signal WebSocket consumer; routes reactions to reward feedback; for text messages, schedules read receipts, enqueues into a bounded receive buffer, and uses one serial worker to debounce same-peer backlog, maintain typing indicators around graph execution, invoke the graph with `thread_id = peer`, and send one overflow reply when the queue is full. After each turn it schedules the interaction review per peer (`_review_jobs`), whose first step stores a durable `pending` row: skipped when the peer has a message waiting (`_InboundMessageBuffer.has_pending`), cancelled when the worker picks up the peer's next message, awaited once it has started writing but cancelled after `_REVIEW_EXECUTION_WAIT_SECONDS` so it never writes after the next turn starts; the cancelling side also finalizes the row (idempotent). `run()`, before consuming the WebSocket, finalizes `executing` rows `error(interrupted)` without replaying them, resumes `pending` rows whose `turn_ref` is still the peer's latest checkpoint, and retires the other pending rows. `wait_for_review(peer)` lets tests settle it
...
- `migrations/0014_interaction_reviews.sql` — Adds `interaction_reviews`, one durable job row per post-send review: `verdict` is the job state `pending|executing|ok|correct|skipped|error`, `action` is `none|complete_task|send_only`, plus `turn_ref` (reviewed checkpoint id), action, target page, executed, follow-up sent, `updated_at`; indexes on `(peer, created_at)` and on unfinished (`pending`/`executing`) rows; `reason` is private text
- `tests/support/` — Shared test doubles: `FakeNotion` (mutable in-memory Notion store; `discard_writes`/`filter_reads` reproduce eval-runner semantics), `SignalSink` (outbound capture; patches the listener's own namespace for receipts/typing), `shame.BANNED_PATTERNS`. Contract-tested by `tests/unit/test_conversation_fakes.py`; `tests/evals/runner.py` builds its Notion stub on top. Also `harness.py` (`Conversation` multi-turn driver, entered through `SignalListener` so `thread_id` derivation and the auth gate are exercised; seeds `active_task` / `recent_tasks`, ages them, delivers reminders and deadline nudges through the real worker, reads `reminder_outbox` state, and settles the post-send interaction review with `settle_review()`) and `invariants.py` (the seven per-turn invariants).
...
- `scripts/ci-local.sh` — Runs the commands CI runs, with CI's env values as defaults: `unit` (ruff/mypy/pytest-unit, no `DATABASE_URL`), `db` (pytest-db against Postgres), `e2e [files…]` (runs pytest under `env -i`, so it receives only `PATH`, `HOME`, `LANG`, `LC_ALL`, `TMPDIR`, `VIRTUAL_ENV`, `PYTHONPATH`, `TERM` from the shell, `ENABLE_E2E_CONVERSATIONS=true`, and `DATABASE_URL`, `LLM_PROXY_BASE_URL`, `LLM_PROXY_API_KEY`, `E2E_MAX_LLM_CALLS`, `E2E_DEBUG_TURNS`, `AUTHORIZED_PEERS`, `SIGNAL_ACCOUNT`, `REWARD_ARTIFACTS_DIR` — from the shell when set, else `e2e.yml`'s value; every other variable, `OPENAI_API_KEY` included, never reaches pytest; refuses to start while `e2e.yml`, `nightly-evals.yml`, or `model-swap.yml` has a queued/in-progress run — they share the LLM proxy's one inference slot — and fails closed when `gh` is missing, unauthenticated, or the lookup fails; `--force` is the only override and every other mode rejects it), `docs` (delegates to `run-required-checks.sh ci-docs`), `all` (unit, db, docs; e2e stays opt-in; a `DATABASE_URL` override reaches the db step). Prints `DATABASE_URL` only as `host:port/dbname`, never with credentials. Never runs the compose smoke test, whose `docker compose down -v` teardown would tear down a developer's own local stack.
