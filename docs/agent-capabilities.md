...
Durable OpenClaw jobs registered from `setup/cron/` with `sessionTarget: isolated`, `payload.kind: agentTurn`, and a concrete model in the canonical cron spec. Routine jobs use `payload.lightContext: true` (empty bootstrap — prompts are self-contained scripts or spec readers) and cheap-tier model. Janitor is the exception: weekly Opus, `payload.lightContext: false`, full bootstrap. Run background work without loading prior user conversation transcripts.
