...
A fifth shape shares the path: a past-tense report of something that was never
on the list dead-ended in the same question.
...
@pytest.mark.asyncio
async def test_an_unlisted_past_tense_report_is_logged_as_done_and_named() -> None:
    """F4: a finished task that was never on the list is logged, not questioned."""
    create_task = AsyncMock(return_value={"id": "<page_new>"})
    update_status = AsyncMock()
    reward = AsyncMock(return_value=_REWARD)
    model = _model(json.dumps({
        "matched_page_id": None,
        "unlisted_completion_title": "Pay the gas bill",
        "confidence": 0.95,
    }))
    with (
        ...
        patch("app.tools.notion.update_status", update_status),
        patch("app.tools.notion.query_all", AsyncMock(return_value={"results": [
            _notion_page("<page_A>", "Book the dentist"),
        ]})),
        patch("app.tools.rewards.maybe_reward", reward),
        patch.object(complete_module, "_load_recent_outbound_target", AsyncMock(return_value=None)),
        patch("app.models.llm", return_value=model),
    ...
    bound = inspect.signature(notion.create_task).bind(*call.args, **call.kwargs)
    assert bound.arguments["title"] == "Pay the gas bill"
    assert bound.arguments["status"] == "Completed"
    update_status.assert_not_awaited()
    _assert_reward_kwargs(reward, "<page_new>", "Pay the gas bill")

    draft = result["pending_outbound"][0]
    assert draft["notion_page_title"] == "Pay the gas bill"
...
@pytest.mark.parametrize(
    ("incoming", "model_reply"),
    [
        # A bare "done" never reaches the model at all.
        ("done!", {"matched_page_id": None, "unlisted_completion_title": "Do it", "confidence": 1}),
        # The model picked a candidate, so nothing new is created.
        ("paid the dentist bill", {
            "matched_page_id": "<page_A>",
            "unlisted_completion_title": "Pay the dentist bill",
            "confidence": 0.95,
        }),
        # The message's words reach an open task: it is about that task, not a new one.
        ("paid the dentist bill", {
            "matched_page_id": None,
            "unlisted_completion_title": "Pay the dentist bill",
            "confidence": 0.95,
        ...
        # A title sharing no word with the message is not the user's task.
        ("paid the gas bill", {
            "matched_page_id": None,
            "unlisted_completion_title": "Walk the dog",
            "confidence": 0.99,
        }),
    ],
)
@pytest.mark.asyncio
async def test_an_unlisted_completion_is_never_invented(
    incoming: str, model_reply: dict[str, Any]
) -> None:
    create_task = AsyncMock(return_value={"id": "<page_new>"})
    with (
        patch("app.tools.notion.create_task", create_task),
        ...
        patch("app.tools.notion.query_all", AsyncMock(return_value={"results": [
            _notion_page("<page_A>", "Pay the dentist bill"),
        ]})),
        patch("app.tools.rewards.maybe_reward", AsyncMock(return_value=_REWARD)),
        patch.object(complete_module, "_load_recent_outbound_target", AsyncMock(return_value=None)),
        patch("app.models.llm", return_value=_model(json.dumps(model_reply))),
    ):
        await complete_module.complete_node(_state(incoming))
