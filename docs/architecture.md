...
flowchart TB
    ...
    subgraph Scheduling["OpenClaw Scheduling"]
        ...
        ReminderCron[Reminder Cron<br/>every 30m]
        PullMainCron[Pull-Main Cron<br/>every 2h]
    end
...
sequenceDiagram
    ...
    Notion-->>Main: page_id
    Main->>Cron: openclaw cron add name=reminder-<page_id> kind=at deleteAfterRun=true
    Main->>User: "Got it — around 6pm: email Melanie"
...
1. At task intake, AI detects reminder language (e.g., "remind me at 6pm PT to call Sarah"), sets `is_reminder = true`, `remind_at` (full ISO 8601 with timezone), `reminder_status = pending`. After `notion-cli.sh create-reminder` returns the Notion `page_id`, the same intake turn uses `exec` and `openclaw cron add` to register a one-shot job named `reminder-<page_id>` with `schedule.kind: "at"`, `at: remind_at`, `deleteAfterRun: true`, `sessionTarget: main`. The CLI path creates a working cron but does not increment OpenClaw's `successfulCronAdds` turn counter, so user-facing confirmations must avoid first-person reminder/scheduling phrases that trigger `agent-runner-reminder-guard`. Success confirmations intentionally say "around <time>" so intake does not overpromise exact wall-clock delivery if the safety-net path ever has to catch a stranded reminder later. See `setup/cron/reminder-delivery.md` for the full contract.
...
| Scheduled Reminders | OpenClaw native one-shot cron (`schedule.kind: at`, `deleteAfterRun: true`) registered at intake, with recurring `reminder-check` poll + `.reminder-signal` handoff as safety net | One-shot fires at exact `remind_at`; safety-net poll catches registration failures or unfired jobs |
| Workspace Sync | OpenClaw durable cron + pull-main.sh | Isolated cron every 2 hours keeps workspace current, recovers dirty pulls |
