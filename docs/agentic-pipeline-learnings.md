...
**v2 (new `review-entry.yml` graph, active when `vars.REVIEW_PIPELINE_V2 == 'true'`):** Two states — GO / NO-GO. The cost-control mechanism v1 needed three states for is now structural: the v2 fixer runs *after* reviewers and *before* the judge, the judge has `permissions: contents: read` and cannot push, and the fixer publishes any new commit first and writes the `review/pipeline` status immediately afterward because GitHub will not accept a status for an unpublished SHA. Re-review loops on finalized GO SHAs are prevented by the GO-only `All Required Agent Reviews = success` short-circuit at entry, so the third state isn't needed. NO-GO in v2 is the human-escalation path: it labels the PR `needs-human-review`, posts one sticky comment, and **does not** close the PR or auto-create a replacement issue (avoids the lessons-learned-issue → new PR → NO-GO infinite loop class). PR #343 introduces this; PR with the gate flip (Phase 2/3) makes it active.
...
**Why:** In v2, the fixer must push before it can claim `review/pipeline`, because GitHub's commit-status API rejects unpublished SHAs. That leaves a bounded synchronize-event race: the `review-entry-v2-<head_ref>` concurrency group serializes the synchronize event behind the still-running cycle, and by the time the queued entry runs the post-fix SHA may already have moved from `review/pipeline = pending` to `success`. Without an additional GO-only short-circuit, the queued entry can re-review the exact SHA cycle N already finalized. On drift-sensitive PRs (anything touching spec-critical Markdown), that redundant cycle can legitimately find fresh cross-doc drift introduced by cycle N's fixer — see PR NickBorgersProbably/hide-my-list#397, which burned 3 full cycles with monotonically diminishing blocker counts (6 → 3 → 1).
...
**Why:** Multi-line bodies in Actions outputs cause shell-escaping failures and context loss. The default pattern is to pass only stable identifiers such as `COMMENT_ID`, `ISSUE_NUMBER`, and `RUN_ID`, then `gh api` the full body inside the devcontainer. The current narrow exception is current-PR metadata that the prompt needs immediately: reviewer/fixer jobs may receive base64-encoded PR title/body so they can evaluate author intent without making another API round-trip, but future refactors should keep that exception tightly scoped instead of turning body handoff into the default pattern.
**Evidence:** #41, #178
