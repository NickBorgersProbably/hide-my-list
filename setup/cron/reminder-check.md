...
Both delivery paths validate the handoff file (JSON with a `reminders` array, each entry having `page_id`, `title`, and `status`), then send each reminder to Signal using the OpenClaw `message` tool (`action: send`, `channel: signal`), then call `scripts/notion-cli.sh complete-reminder PAGE_ID sent|missed` to atomically set Notion `Status` to `Completed`, `Reminder Status` to `sent` or `missed`, and `Completed At`. Malformed handoff files are left in place for operator attention.
