...
Check 1 safety-net reminder status is produced upstream by `scripts/check-reminders.sh`: it compares UTC instants and writes due reminders with `status: sent`. Heartbeat consumes that handoff and accepts legacy `missed` entries without recomputing `now - Remind At`. Use the helper only when delivery wording needs user-local phrasing derived from `remind_at` or from the current local date.
...
Check via `openclaw cron list --json`. Missing → re-create with `openclaw cron add` (durable: true) using schedule, prompt, options from `setup/cron/`. Canonical fields come from `setup/cron/<name>.md`: `sessionTarget`, exact `model:` line, `payload.kind`, `payload.lightContext`, and `timeout-seconds`. `heartbeat`, `reminder-check`, `reminder-delivery-sweep`, and `pull-main` must match the cheap tier in `setup/model-tiers.json`; `janitor` is decoupled from cheap tier and uses its explicit Opus model for weekly deep audits. Cron jobs never use direct-delivery routing; heartbeat Check 1 and `reminder-delivery-sweep` use explicit `message` tool calls for reminder delivery and ops alerts. Current OpenClaw durable cron registration uses `agentTurn`; reduced `reminder-check` and `pull-main` cadence limits routine LLM use.
...
- Verify the full OpenClaw media staging path is traversable for Signal attachment delivery:
  ...
  scripts/ensure-openclaw-media-staging.sh
...
Check: `openclaw cron list --json` for live registrations, read spec files in `setup/cron/`.
...
Stale `pipeline-monitor` cron still registered → delete with the CLI-supported cron delete command (job removed).

Field differs from spec → patch with the CLI-supported cron update command. Identity field (`name`, `durable`) can't be safely changed → delete + re-create from spec. Intended contract:
