...
- The heartbeat is also the hourly backstop for reminder delivery. The isolated `reminder-check` cron only writes `.reminder-signal`; heartbeat Check 1 reads stranded reminders, delivers them with the `message` tool (`action: send`, `channel: signal`, `target: channels.signal.defaultTo` from `openclaw.json`), and only then marks them complete. `TOOLS.md` defines the canonical send-success contract.
