...
sequenceDiagram
    ...
    Main->>User: "Got it — reminder set for around 6pm"
...
1. At task intake, AI detects reminder language (e.g., "remind me at 6pm PT to call Sarah"), sets `is_reminder = true`, `remind_at` (full ISO 8601 with timezone), `reminder_status = pending`. After `notion-cli.sh create-reminder` returns the Notion `page_id`, the same intake turn registers a one-shot job named `reminder-<page_id>` with `schedule.kind: "at"`, `at: remind_at`, `deleteAfterRun: true`, `sessionTarget: main`. Native `CronCreate` calls satisfy OpenClaw's `agent-runner-reminder-guard`; shell `openclaw cron add` registrations still fire correctly but may not increment `successfulCronAdds`, so reminder confirmations must also avoid phrases like `"I'll remind you ..."` in the user-visible reply. See `setup/cron/reminder-delivery.md` for the full contract.
