...
#### Proxy w/ domain allowlist

The host has local firewall configured which blocks direct egress via HTTP/S. This is intended to reduce the risk that the agent reaches out to a webserver for information and consumes a prompt injection attack. As described above, there is a carve out for GitHub - a non-substantial tradeoff made to create an interative loop of continuous improvement.
...
A Squid proxy enforces a domain allowlist. If the agent (or anything else on the host) tries to reach a domain that isn't explicitly allowed, the connection is denied. This provides mitigation against prompt injectino because the agent will not go consuming aribtrary web content.
