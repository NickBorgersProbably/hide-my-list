...
Every 60 minutes, OpenClaw creates a short agent session that reads `HEARTBEAT.md` and executes the checks defined there. It uses a lighter model (Sonnet instead of Opus) since these are routine operational tasks. Reminder delivery does not rely on `heartbeat.target`; Check 1 sends reminders explicitly with the OpenClaw `message` tool (`action: send`, `channel: signal`). Ops alerts go to a separate operator recipient via `message(action: send, channel: signal, target: OPS_NUMBER)`, where `OPS_NUMBER` is read from `OPS_ALERT_SIGNAL_NUMBER` in `.env`. The `target` field in the `heartbeat` config block is intentionally absent — generic non-`HEARTBEAT_OK` output is silently discarded, and all meaningful alerts are routed explicitly ([openclaw/openclaw#29215](https://github.com/openclaw/openclaw/issues/29215)).
