...
Because the job re-enters `sessionTarget: main`, outbound routing is deterministic: deliver reminders only through the user-facing surface already attached to that main session. Do not pick a different recipient, channel, or thread. Resolve the reminder handoff file with the same shell helper and repo-root filename validation the scripts use so `.env` overrides are honored:
...
(the `REMINDER_SIGNAL_FILE` env var is honored; default path is `.reminder-signal`)
...
After delivery, read each reminder's status from the handoff file and update Notion accordingly:
  If status is "sent":
    scripts/notion-cli.sh complete-reminder PAGE_ID sent
  If status is "missed":
    scripts/notion-cli.sh complete-reminder PAGE_ID missed
Delete `HANDOFF_FILE` only after every reminder was delivered and `scripts/notion-cli.sh complete-reminder` updated Notion for each item.
