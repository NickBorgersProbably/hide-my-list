...
"this focus task" with no name. Placeholder titles and ids only.
"""
...
async def _run(pages: list[dict[str, Any]], payload: dict[str, Any]) -> tuple[dict[str, Any], AsyncMock, list[Any]]:
...
async def test_token_without_selection_gets_the_no_match_reply() -> None:
    ...
    _assert_no_selection(result, update_status)
    assert "Nothing quite fits" in result["pending_outbound"][0]["body"]
