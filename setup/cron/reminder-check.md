...
Deliberate design: `sessionTarget: isolated` with cheap-tier model keeps per-run cost low. A `sessionTarget: main` full Opus context (~200k tokens) for a job that 95% finds nothing would be orders of magnitude more expensive. Trade-off: fully idle safety-net delivery can wait until the daily heartbeat. In practice most reminders still deliver through the primary one-shot cron or on next user interaction.
