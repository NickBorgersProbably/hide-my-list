...
- `docs/ai-prompts/interaction-review.md` — Post-send interaction review: inputs, strict JSON verdict schema, correction policy, shame rules for the follow-up
...
- `app/tools/interaction_reviews.py` — Interaction review job store: `create_pending` (durable row before a review runs), `finalize` (idempotent move to `ok|correct|skipped|error`; no-op on a final row), `list_pending` (startup resume), and `count_executed_corrections` (per-peer hourly rate limit, all-peer 24 h alert count). The review's only Postgres access
...
- `app/graph/interaction_review.py` — Post-send interaction review, run by the Signal listener as a background task after each turn (not a graph node). `review_turn` reads the finished turn, calls the medium model (`judge_turn`, caller `interaction_review`), validates the strict JSON verdict (`parse_verdict`: action enum, page ids from the shown lists only, `reopen_task` only for this turn's completions, `{task}` in every follow-up, no blame phrasing), runs at most one action, sends one follow-up through `render_task_token`, and writes the checkpoint as the `send` node only while the thread's latest checkpoint id still equals `turn_ref` (else `error(stale_checkpoint)`). Every exit path finalizes the job row passed in as `review_id`, cancellation included (skip reasons `buffer_non_empty|superseded|cancelled|rate_limited|disabled|timeout`). Rate limit + ops alert + job store via `app/tools/interaction_reviews.py`
...
- `migrations/0014_interaction_reviews.sql` — Adds `interaction_reviews`, one durable job row per post-send review: `verdict` is the job state `pending|ok|correct|skipped|error`, plus `turn_ref` (reviewed checkpoint id), action, target page, executed, follow-up sent, `updated_at`; indexes on `(peer, created_at)` and on pending rows; `reason` is private text
