...
- `app/graph/nodes/_log_finished.py` — Shared "log an accomplishment" path used by intake (`already_finished` saves) and complete (a yes to the unlisted-report log question): completes the open task a title duplicates at the intake dedup threshold or creates the page Completed, calls `maybe_reward` with complete_node's kwargs, records a `completed` ledger event, bumps streak/`tasks_completed_today`, clears `pending_clarification`, and replies with `_celebration_body` + `notion_page_title`. Logs booleans/counts only under the caller's event name
