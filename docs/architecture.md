...
| `notion_health` | 15 min | Check Notion API connectivity; enqueue warning `notion_health_failed` on connectivity failure; when connectivity succeeds, validate task database schema and enqueue critical `notion_schema_mismatch` for missing or mistyped required properties |
