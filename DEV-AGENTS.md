...
- `scripts/ci-local.sh` — Runs exactly what CI runs, in CI's environment: `unit` (ruff/mypy/pytest-unit, no `DATABASE_URL`), `db` (pytest-db against Postgres), `e2e [files…]` (`e2e.yml`'s exact env, always unsets `OPENAI_API_KEY`, refuses to start while `e2e.yml` is running in CI — the LLM proxy has one inference slot — unless `--force`), `docs` (delegates to `run-required-checks.sh ci-docs`), `all` (unit, db, docs; e2e stays opt-in). Never runs the compose smoke test, whose `docker compose down -v` teardown would tear down a developer's own local stack.
