...
- `scripts/ci-local.sh` — Runs the commands CI runs, with CI's env values as defaults: `unit` (ruff/mypy/pytest-unit, no `DATABASE_URL`), `db` (pytest-db against Postgres), `e2e [files…]` (sets `e2e.yml`'s values as defaults; only `DATABASE_URL`, `LLM_PROXY_BASE_URL`, `LLM_PROXY_API_KEY`, `E2E_MAX_LLM_CALLS`, `E2E_DEBUG_TURNS`, `AUTHORIZED_PEERS`, `SIGNAL_ACCOUNT`, `REWARD_ARTIFACTS_DIR` may be overridden from the shell; `OPENAI_API_KEY` is always unset and `ENABLE_E2E_CONVERSATIONS` always `true`; refuses to start while `e2e.yml`, `nightly-evals.yml`, or `model-swap.yml` has a queued/in-progress run — they share the LLM proxy's one inference slot — and fails closed when `gh` is missing, unauthenticated, or the lookup fails; `--force` is the only override), `docs` (delegates to `run-required-checks.sh ci-docs`), `all` (unit, db, docs; e2e stays opt-in). Never runs the compose smoke test, whose `docker compose down -v` teardown would tear down a developer's own local stack.
