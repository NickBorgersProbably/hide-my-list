...
3. **Operator alerting:** On any failure path (malformed handoff, Notion down, env missing, dirty-pull stuck), heartbeat sends an explicit ops alert to the operator's Signal number. Caroline's thread only ever sees reminders — never raw infrastructure failures.
